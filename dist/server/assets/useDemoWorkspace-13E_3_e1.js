...
//#region src/domain/intake-matching.ts
...
function findPersonIntakeMatches({ cases, input, people, programs, staff }) {
	...
	return people.reduce((matches, person) => {
		const nameScore = firstName && normalize(person.firstName).startsWith(firstName) || lastName && normalize(person.lastName).startsWith(lastName);
		const exactContact = phone && normalizePhone(person.phone) === phone || email && normalize(person.email) === email;
		const exactIdentity = input.dateOfBirth && person.dateOfBirth === input.dateOfBirth;
		if (!nameScore && !exactContact && !exactIdentity) return matches;
		const caseRecord = cases.find((item) => item.personId === person.id);
		const firstEnrollment = caseRecord?.enrollments[0];
		const program = firstEnrollment ? getProgram(programs, firstEnrollment.programId) : void 0;
		const primaryStaffId = firstEnrollment?.caseworkers.find((assignment) => assignment.isPrimary)?.staffId;
		const clientName = [person.firstName, person.lastName].filter(Boolean).join(" ").trim();
		matches.push({
			id: person.id,
			personId: person.id,
			...
			strength: exactContact || exactIdentity ? "High confidence" : firstName && lastName && nameScore ? "Medium confidence" : "Low confidence",
			action: caseRecord ? caseRecord.status === "Closed" ? "reintake" : "view_case" : "start_intake"
		});
		return matches;
	}, []);
}
...
function DemoWorkspaceProvider({ children }) {
	...
	async function createCaseFromIntake(input, options) {
